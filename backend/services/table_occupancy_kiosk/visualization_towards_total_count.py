"""
Visualization for Total Occupancy Count - KIOSK
...
REGION_KEY = os.getenv("REGION_KEY", "kiosk")
...
def visualize_total_occupancy():
    ...
    while True:
        try:
            ...
            if msg is None:
                if window_initialized:
                    if cv2.waitKey(1) & 0xFF == ord('q'):
                        break
                    try:
                        if cv2.getWindowProperty("Total Occupancy Visualization - KIOSK", cv2.WND_PROP_VISIBLE) < 1:
                            break
                    except:
                        break
                continue
            ...
            cv2.imshow("Total Occupancy Visualization - KIOSK", frame)
