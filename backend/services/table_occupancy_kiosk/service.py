...
class TableFrameProcessor:
    def __init__(
        ...
        self.occupancy_regions = load_region_config(region_key="kiosk")
        logger.info(
    ...
    def _load_configs(self):
        ...
        try:
            with open(self.config_path, "r") as f:
                self.table_configs = json.load(f)["kiosk"]
                ...
                if (
                    ...
                    if config_changed:
                        ...
                        try:
                            ...
                            full_config["kiosk"] = self.table_configs
            ...
            if (
                ...
                try:
                    ...
                    ticket = {
                        "camera_id": camera_id,
                        "timestamp": timestamp,
                        "description": f"Table {index} detected as dirty in kiosk region",
                        "type": "raised",
                        "ticket_id": new_ticket_id,
                        "target_collection": "alert_collection",
                        "frame": frame_b64,
            ...
            # Resolve Dirty Ticket if State became EMPTY (from DIRTY)
                ...
                try:
                    ...
                    ticket = {
                        "camera_id": camera_id,
                        "timestamp": timestamp,
                        "description": f"Table {index} marked as clean in kiosk region",
                        "type": "resolved",
                        "ticket_id": existing_ticket_id,
                        "target_collection": "alert_collection",
                        "frame": frame_b64,
        ...
        if dirty_table_count > 4 and not self.many_dirty_ticket:
            try:
                ...
                ticket = {
                    "camera_id": camera_id,
                    "timestamp": timestamp,
                    "description": f"High number of dirty tables detected in kiosk region (More than 4 tables)",
                    "type": "raised",
                    "ticket_id": new_ticket_id,
                    "target_collection": "alert_collection",
                    "frame": frame_b64,
            ...
            try:
                ...
                ticket = {
                    "camera_id": camera_id,
                    "timestamp": timestamp,
                    "description": f"High number of dirty tables resolved in kiosk region",
                    "type": "resolved",
                    "ticket_id": existing_ticket_id,
                    "target_collection": "alert_collection",
                    "frame": frame_b64,
