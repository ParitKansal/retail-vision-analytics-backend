...
class TableFrameProcessor:
    def __init__(
        ...
        self.PLATE_OVERLAP_THRESHOLD = 0.5  # 50% of plate must be inside table
        ...
        self.occupancy_regions = load_region_config(region_key="left")
        logger.info(
    ...
    class utility_functions:
        ...
        def detect_with_api(
            ...
            try:
                ...
                api_end = time.time()
                logger.info(
                    f"API call took {(api_end - api_start)*1000:.2f}ms for prompt: '{prompt_text[:50]}'"
                )
            ...
            if len(boxes) == 0:
                logger.info(f"No detections for prompt: '{prompt_text[:50]}'")
                return []

            logger.info(
                f"Got {len(boxes)} raw detections for prompt: '{prompt_text[:50]}'"
            )
            ...
            if apply_nms:
                ...
                nms_end = time.time()
                logger.info(
                    f"NMS took {(nms_end - nms_start)*1000:.2f}ms, filtered to {len(filtered)} detections"
                )
            else:
                ...
                logger.info(f"Skipped NMS, using all {len(filtered)} detections")
            ...
            logger.info(
                f"detect_with_api total: {total_time*1000:.2f}ms, returned {len(final_dets)} detections above threshold {score_thresh}"
            )
    ...
    def _load_configs(self):
        ...
        try:
            with open(self.config_path, "r") as f:
                self.table_configs = json.load(f)["left"]
    ...
    def process_single_frame(self, message):
                ...
                if (
                    ...
                    if config_changed:
                        ...
                        try:
                            ...
                            full_config["left"] = self.table_configs
        ...
        for table_data in self.tables:
            ...
            # Refetched state in case it changed above from Occupied -> Empty
            ...
            if (
                ...
                try:
                    ...
                    ticket = {
                        "camera_id": camera_id,
                        "timestamp": timestamp,
                        "description": f"Table {index} detected as dirty in left lobby region",
                        "type": "raised",
            ...
            # Resolve Dirty Ticket if State became EMPTY (from DIRTY)
                ...
                try:
                    ...
                    ticket = {
                        "camera_id": camera_id,
                        "timestamp": timestamp,
                        "description": f"Table {index} marked as clean in left lobby region",
                        "type": "resolved",
        ...
        if dirty_table_count > 4 and not self.many_dirty_ticket:
            try:
                ...
                ticket = {
                    "camera_id": camera_id,
                    "timestamp": timestamp,
                    "description": f"High number of dirty tables detected in left lobby region (More than 4 tables)",
                    "type": "raised",
                    "ticket_id": new_ticket_id,
                    "target_collection": "alert_collection",
                    "frame": frame_b64,
            ...
            try:
                ...
                ticket = {
                    "camera_id": camera_id,
                    "timestamp": timestamp,
                    "description": f"High number of dirty tables resolved in left lobby region",
                    "type": "resolved",
                    "ticket_id": existing_ticket_id,
                    "target_collection": "alert_collection",
                    "frame": frame_b64,
    ...
    def evaluate_check_block(self, person_box, table_box, check_block):
        ...
        if "table_overlap" in check_block:
            ...
            # The user json shows '0.05', '0.3', '0.5' etc. but 'left' is '69'.
            # I'll treat table_overlap as 0-1 float as per JSON.
    ...
    def check_person_occupancy(self, person_box, table_id, table_box):
        ...
        if not config:
            ...
            # Or return False? Assuming config is required for specific matching.
            # But maybe fallback to global default?
            # Current JSON has many tables. If missing, maybe safe to say False or default IOU.
            # Let's use a default IOU of 0.3 for unconfigured tables just in case.
