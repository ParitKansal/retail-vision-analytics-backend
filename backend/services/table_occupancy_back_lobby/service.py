...
PERCENTAGE_DIRTY_IN_BUFFER = int(os.getenv("PERCENTAGE_DIRTY_IN_BUFFER", 50))
PERCENTAGE_OCCUPIED_IN_BUFFER = int(os.getenv("PERCENTAGE_OCCUPIED_IN_BUFFER", 50))
CONSECUTIVE_CLEAN_FOR_RESOLUTION = int(
    os.getenv("CONSECUTIVE_CLEAN_FOR_RESOLUTION", 30)
)
CONSECUTIVE_UNOCCUPIED_FOR_RESOLUTION = int(
    os.getenv("CONSECUTIVE_UNOCCUPIED_FOR_RESOLUTION", 30)
)
...
class TableFrameProcessor:
    def __init__(
        ...
        self.occupancy_regions = load_region_config(region_key="back")
        logger.info(
        ...
        self.CONFIG_UPDATE_IOU_THRESHOLD = float(os.getenv("CONFIG_UPDATE_IOU_THRESHOLD", 0.8))  # 80% overlap
    ...
    class utility_functions:
        ...
        @staticmethod
        def upper_side_overlap(person_box, table_box, min_horizontal_iou=0.1):
            """
    ...
    def _load_configs(self):
        ...
        try:
            with open(self.config_path, "r") as f:
                self.table_configs = json.load(f)["back"]
    ...
    def process_single_frame(self, message):
        ...
        try:
            ...
            try:
                ...
                if (current_time - self.last_config_update_time) >= self.CONFIG_UPDATE_INTERVAL:
                    ...
                    for t_id, config_bbox in self.table_configs.items():
                        ...
                        for det_bbox in detected_tables:
                             ...
                             iou = self.utility_functions.calculate_iou(config_bbox, det_bbox)
                             if iou > best_iou:
                                 best_iou = iou
                        ...
                        if best_match_bbox is not None and best_iou > 0.1: # Ensure it's roughly the same table (min 10% overlap safety)
                            if best_iou < self.CONFIG_UPDATE_IOU_THRESHOLD:
                                logger.info(f"Table {t_id} overlap {best_iou:.2f} < 0.80. Updating config.")
                                self.table_configs[t_id] = best_match_bbox
                                config_changed = True
                            else:
                                logger.info(f"Table {t_id} overlap {best_iou:.2f} >= 0.80. Keeping existing config.")
                        else:
                             # No matching detection found for this table (e.g. occlusion). Do not update.
                             logger.info(f"Table {t_id} has no matching detection. Skipping update.")
                    ...
                    if config_changed:
                        ...
                        try:
                            ...
                            # We need to read the full file first to preserve other sections if any (e.g. if this file was shared, though here it implies 'back' is the key)
                            # To be safe, we re-read or just assume the structure we know.
                            # self.table_configs is just the 'back' part.
                            
                            # Re-read to ensure we don't overwrite other keys if they exist in the file but not in memory
                            with open(config_path, "r") as f:
                                full_config = json.load(f)
                            
                            full_config["back"] = self.table_configs
        ...
        for p in persons:
            ...
            for table_data in self.tables:
                ...
                if self.check_person_occupancy(p["bbox"], t_id, t_bbox):
                    ...
                    prio = int(self.checks_config.get(t_id, {}).get("priority", 0))
        ...
        for table_data in self.tables:
            ...
            # Calculate consecutive stats for visualization (Iterate backwards)
            consec_empty = 0
            ...
            self.debug_table_stats[index] = {
                "person_count": person_count_in_history,
                "tray_count": tray_count_in_history,
                "history_len": len(history),
                "consec_empty": consec_empty,
                "consec_clean": consec_clean,
                "occupied_threshold": self.OCCUPIED_TRIGGER_THRESHOLD,
                "dirty_threshold": self.DIRTY_TRIGGER_THRESHOLD,
                "resolve_empty_threshold": self.CONSECUTIVE_UNOCCUPIED_FOR_RESOLUTION,
                "resolve_clean_threshold": self.CONSECUTIVE_CLEAN_FOR_RESOLUTION
            }
            ...
            # --- OCCUPIED Alert Logic --- (COMMENTED OUT)
            # occupied_ticket = tickets["occupied"]
            #
            # # Raise Occupied Ticket if State became OCCUPIED and no ticket exists
            # if self.current_table_state[index] == TableStatus.OCCUPIED.name and not occupied_ticket:
            #     try:
            ...
            #                 frame_cv = self.draw_visualizations(frame_cv, self.tables, self.table_status, persons, plates)
            ...
            #             frame_b64 = base64.b64encode(img_response.content).decode("utf-8")
            #         logging.info(f"occupied ticket raised for table {index}")
            #
            ...
            #             "camera_id": camera_id,
            #             "timestamp": timestamp,
            #             "description": f"occupied with table {index}",
            #             "type": "raised",
            ...
            #         }
            #
            #         self.open_tickets[index]["occupied"] = new_ticket_id
            #
            #         self.redis.xadd(OUTPUT_QUEUE_1, {"data": json.dumps(ticket)})
            #         logger.info(
            #             f"Published occupied alert for camera {camera_id} at {timestamp}"
            #         )
            #     except Exception as e:
            #         logger.error(f"Failed to raise occupied ticket: {e}")
            #
            # # Resolve Occupied Ticket if State became EMPTY (from OCCUPIED)
            # elif occupied_ticket and self.current_table_state[index] == TableStatus.EMPTY.name:
            #     try:
            #         img_response = requests.get(frame_url, timeout=10)
            #         img_response.raise_for_status()
            #         if len(img_response.content) == 0:
            ...
            #             frame_b64 = base64.b64encode(img_response.content).decode("utf-8")
            #
            #         logging.info(f"occupied ticket resolved for table {index}")
            #
            #         existing_ticket_id = self.open_tickets[index]["occupied"]
            #
            #         ticket = {
            #             "camera_id": camera_id,
            #             "timestamp": timestamp,
            #             "description": f"empty with table {index}",
            #             "type": "resolved",
            #             "ticket_id": existing_ticket_id,
            #             "target_collection": "alert_collection",
            #             "frame": frame_b64,
            #         }
            #         self.open_tickets[index]["occupied"] = None
            #         self.redis.xadd(OUTPUT_QUEUE_1, {"data": json.dumps(ticket)})
            #         logger.info(
            #             f"Published occupied resolution for camera {camera_id} at {timestamp}"
            #         )
            #     except Exception as e:
            #         logger.error(f"Failed to resolve occupied ticket: {e}")
            ...
            if (
                ...
                try:
                    ...
                    ticket = {
                        "camera_id": camera_id,
                        "timestamp": timestamp,
                        "description": f"Table {index} detected as dirty in back lobby region",
                        "type": "raised",
                        "ticket_id": new_ticket_id,
                        "target_collection": "alert_collection",
                        "frame": frame_b64,
            ...
            # Resolve Dirty Ticket if State became EMPTY (from DIRTY)
                ...
                try:
                    ...
                    ticket = {
                        "camera_id": camera_id,
                        "timestamp": timestamp,
                        "description": f"Table {index} marked as clean in back lobby region",
                        "type": "resolved",
                        "ticket_id": existing_ticket_id,
                        "target_collection": "alert_collection",
                        "frame": frame_b64,
        ...
        if dirty_table_count > 4 and not self.many_dirty_ticket:
            try:
                ...
                ticket = {
                    "camera_id": camera_id,
                    "timestamp": timestamp,
                    "description": f"High number of dirty tables detected in back lobby region (More than 4 tables)",
                    "type": "raised",
                    "ticket_id": new_ticket_id,
                    "target_collection": "alert_collection",
                    "frame": frame_b64,
            ...
            try:
                ...
                ticket = {
                    "camera_id": camera_id,
                    "timestamp": timestamp,
                    "description": f"High number of dirty tables resolved in back lobby region",
                    "type": "resolved",
                    "ticket_id": existing_ticket_id,
                    "target_collection": "alert_collection",
                    "frame": frame_b64,
                    ...
                    vis_payload = {
                        **output,
                        ...
                        "config": {
                            ...
                            "PERCENTAGE_DIRTY_IN_BUFFER": PERCENTAGE_DIRTY_IN_BUFFER,
                            "PERCENTAGE_OCCUPIED_IN_BUFFER": PERCENTAGE_OCCUPIED_IN_BUFFER,
                            "CONSECUTIVE_CLEAN_FOR_RESOLUTION": CONSECUTIVE_CLEAN_FOR_RESOLUTION,
                            "CONSECUTIVE_UNOCCUPIED_FOR_RESOLUTION": CONSECUTIVE_UNOCCUPIED_FOR_RESOLUTION,
                        },
                        "table_debug_info": self.debug_table_stats
                    }
