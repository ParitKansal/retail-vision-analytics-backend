...
def visualize():
    ...
    while True:
        try:
            ...
            # Check if window was closed (only if created)
            # Use a short timeout so we can check UI events frequently
            ...
            # Consume ALL available frames to get to the latest one
            # Use short timeout to allow UI processing
            msg = r.blpop(QUEUE_NAME, timeout=0.1)
            
            # If manually closed (window prop < 1), we want to break.
            # But we can also check getWindowProperty here if we track if it was opened.
            # The check above handles it if the window exists. 
            # If msg is None (timeout), we loop back and check again.
            
            if msg is None:
                # If window was somehow closed during timeout, we detect it next loop
                # If window was never created, we just wait.
                # If window WAS created and user closed it, getWindowProperty above might fail if window is destroyed?
                # Actually getWindowProperty returns -1 or 0 if window is closed/doesn't exist.
                # But we can only check it safely if we know we opened it?
                # cv2.getWindowProperty is safe to call on non-existent windows? 
                # usually it returns 0 or -1.
                # But let's be safe: invoke waitKey to process events.
                
                # Check for Q or Close if window is known to govern logic
                try:
                     if cv2.getWindowProperty("Table Occupancy Visualization", cv2.WND_PROP_VISIBLE) < 1:
                          # Only break if we have previously shown the window? 
                          # If we haven't shown it yet, user can't close it, so this check is checking "non-existent" window.
                          # If "non-existent", returns 0. So it would break immediately on start!
                          # We MUST track if window was opened.
                          pass
                except:
                     pass
                continue
