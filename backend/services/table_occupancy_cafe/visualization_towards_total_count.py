"""
Visualization for Total Occupancy Count - CAFE
...
REGION_KEY = os.getenv("REGION_KEY", "cafe")
...
def visualize_total_occupancy():
    ...
    while True:
        try:
            ...
            if msg is None:
                if window_initialized:
                    if cv2.waitKey(1) & 0xFF == ord("q"):
                        break
                    try:
                        if cv2.getWindowProperty("Total Occupancy Visualization - CAFE", cv2.WND_PROP_VISIBLE) < 1:
                            break
                    except Exception:
                        break
                continue
            ...
            cv2.imshow("Total Occupancy Visualization - CAFE", frame)
